...
function getFrameAt(
    ...
    if (frameIdx < horse.timeline.length) {
        ...
        return {
            distance: f.distance,
            speed: f.speed,
            finished: f.distance >= raceDistanceM,
        };
    }
...
export function precomputeRace(
    ...
    for (let tick = 0; tick < totalTicks; tick++) {
        ...
        const elapsedMs = Math.min(
            frameIdx * FRAME_INTERVAL_MS,
            replays.reduce(
                (max, r) =>
                    Math.max(max, r.timeline[r.timeline.length - 1]?.time ?? 0),
                0,
            ),
        );
        ...
        ticks.push({
            ...
            horses: replays.map((r) => {
                ...
                return {
                    ...
                    progressPct: Number(
                        ((frame.distance / raceDistanceM) * 100).toFixed(2),
                    ),
    ...
    const totalDurationMs =
        ticks.length > 0 ? ticks[ticks.length - 1]!.elapsedMs : 0;
